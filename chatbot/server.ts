...
function parseAgendarFromMessage(text: string): { fechaExpr: string; hora: string; motivo: string; tipo_consulta: string } | null {
  ...
  let motivo = "";
  const motivoMatch = t.match(/(?:motivo|moitvo|con el motivo|con el moitvo)\s*["']?([^"'\n]+)["']?/i) || t.match(/"([^"]+)"/);
...
/** Lista limitada + /patients/search para no fallar si el paciente no está en el primer lote de 500. */
...
async function runTool(token: string, name: string, args: Record<string, unknown>): Promise<Record<string, unknown> & { navigateTo?: string }> {
  try {
    switch (name) {
      ...
      case "nuevo_paciente": {
        const r = await backend.createPatient(token, {
          nombres: String(args.nombres ?? ""), apellidos: String(args.apellidos ?? ""), cedula: String(args.cedula ?? ""),
      ...
      case "agendar_consulta": {
        ...
        const hora = String(args.hora ?? "").trim(), motivo = String(args.motivo ?? "").trim();
        ...
        const r = await backend.createConsulta(token, { paciente_id, medico_id, motivo_consulta: motivo, fecha_pautada: fecha, hora_pautada: hora, tipo_consulta });
        return r.success ? { success: true, data: r.data, message: "Consulta agendada correctamente." } : { success: false, error: r.error };
      }
      ...
      case "crear_recipe_medico": {
        ...
        if (!textoIndicaciones || lineasNombres.length === 0) {
          return {
            ...
            error: {
              message:
                "Se requieren **nombres_medicamentos** (un medicamento por línea, solo el nombre) y **texto_indicaciones**.",
            },
          };
        }
        ...
        const message =
          "Se generaron **dos PDF**: medicamentos (solo nombres) e indicaciones (texto completo). Indica los botones **PDF medicamentos** y **PDF indicaciones**.";
        return {
          success: true,
          message,
          pdf_base64_recipe,
...
async function handleMessage(req: Request): Promise<Response> {
  ...
  if (confirmacionAgendar) {
    ...
    const executeTool = async (name: string, args: Record<string, unknown>) => {
      const raw = (await runTool(token, name, args)) as Record<string, unknown>;
    ...
    if (result.action && parsedData) {
      const { toolName, args } = legacyActionToTool(result.action, parsedData);
      if (toolName) {
        const toolResult = await runTool(token, toolName, args) as Record<string, unknown>;
...
async function handler(req: Request): Promise<Response> {
  ...
  if (url.pathname === "/health" && req.method === "GET") {
    return new Response(JSON.stringify({ ok: true, service: "dr-anderson-chatbot" }), {
      headers: { ...corsHeaders, "Content-Type": "application/json" },
    });
  }
...
console.log(`🤖 Chatbot Dr. Anderson Cepeda escuchando en http://0.0.0.0:${PORT}`);
